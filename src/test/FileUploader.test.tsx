...
describe('FileUploader', () => {
  ...
  it('should render upload interface in default state', () => {
    ...
    // Idle state shows the upload icon as the illustration and again inside the button
  ...
  it('should have hidden file input with correct attributes', () => {
    ...
    const fileInput = screen.getByLabelText('Upload XML file') as HTMLInputElement;
  ...
  it('should have proper drop-zone styling classes', () => {
    ...
    expect(uploaderDiv).toHaveClass('border-2', 'border-dashed', 'cursor-pointer');
  });

  it('should show a dragging state on drag over and clear it on drag leave', () => {
    const { container } = render(<FileUploader onFileUpload={mockOnFileUpload} isLoading={false} error={null} />);
  ...
  it('should call onFileUpload when a file is dropped on the zone', () => {
    const { container } = render(<FileUploader onFileUpload={mockOnFileUpload} isLoading={false} error={null} />);

    const dropZone = container.firstChild as HTMLElement;
    const file = new File(['dropped content'], 'dropped.xml', { type: 'text/xml' });
  ...
  it('should not call onFileUpload for a drop while loading', () => {
    const { container } = render(<FileUploader onFileUpload={mockOnFileUpload} isLoading={true} error={null} />);
